...
class Gitlab3Service(IssueService):
    ISSUE_CLASS = Gitlab3Issue
    CONFIG_PREFIX = 'gitlab3'

    def __init__(self, *args, **kw):
        super(Gitlab3Service, self).__init__(*args, **kw)
        ...
        self.exclude_repos = []
        if self.config.get('exclude_repos', None):
            self.exclude_repos = [
                item.strip() for item in
                self.config.get('exclude_repos').strip().split(',')
            ]

        self.include_repos = []
        if self.config.get('include_repos', None):
            self.include_repos = [
                item.strip() for item in
                self.config.get('include_repos').strip().split(',')
            ]
    ...
    def validate_config(cls, config, target):
        if not config.has_option(target, 'gitlab3.host'):
            die("[%s] has no 'gitlab3.host'" % target)

        if not config.has_option(target, 'gitlab3.login'):
            die("[%s] has no 'gitlab3.login'" % target)

        if not config.has_option(target, 'gitlab3.token'):
            die("[%s] has no 'gitlab3.token'" % target)

        super(Gitlab3Service, cls).validate_config(config, target)
